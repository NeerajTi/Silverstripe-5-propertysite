...
class OwnerDashboardController extends ContentController {
    private static $allowed_actions = [
        'index','info','updateinfo','savenotificationalert','tips','remove_account','schufa','subscription','payment_method','payment_discount','doUpdateRenter','deleteFileImg','uploadFile','wishlist','applications','application_detail','saved_search'
    ];
...
public function remove_account(HTTPRequest $request){
    ...
    DB::query("DELETE FROM Member WHERE ID = " . $member->ID);
    DB::query("DELETE FROM MemberBasicData WHERE MemberID = " . $member->ID);
    DB::query("DELETE FROM MemberSearchSetting WHERE MemberID = " . $member->ID);
    DB::query("DELETE FROM MemberPersonalInformation WHERE MemberID = " . $member->ID);
    DB::query("DELETE FROM ApartmentApplication WHERE MemberID = " . $member->ID);
    DB::query("DELETE FROM ApartmentWishlist WHERE MemberID = " . $member->ID);
    DB::query("DELETE FROM MemberPassword WHERE MemberID = " . $member->ID);
