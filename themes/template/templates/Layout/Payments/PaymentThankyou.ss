...
<div class="white_card" style="display:block;">
			<div class="container">
				...
				<% if $userType == 'owner' || $userType == 'broker' || $userType == 'seller' %>
				<a href='/dashboard/guide' class="yellow_btn">Weiter</a>
				<% end_if %>
<% if $userType != 'owner' %>
				<a href="/stadtteile-berlins" class="yellow_btn">Weither zur wohnungsuche</a> 
			<% end_if %>
				<a class="outline_btn" href="$Dashboard">zur dashboard</a>
				</div>
			</div>
		</div>
                <% include Footer %>
